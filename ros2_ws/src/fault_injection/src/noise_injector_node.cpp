 ...
 *    The result Z0 has a mean of 0.0 and variance of 1.0. We scale it by
 ...
 *    For packet loss rate p in [0, 1], we draw a uniform variable U in [0, 1].
...
public:
  NoiseInjectorNode() : Node("noise_injector_node") {
    ...
    this->declare_parameter("sensor_noise_stddev", 0.15); // meters of range noise
    ...
    fault_config_sub_ = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&NoiseInjectorNode::config_callback, this, std::placeholders::_1));

    lidar_raw_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
      "/lidar/raw", 10,
      std::bind(&NoiseInjectorNode::lidar_callback, this, std::placeholders::_1));

    odom_raw_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      "/odom/raw", 10,
      std::bind(&NoiseInjectorNode::odom_callback, this, std::placeholders::_1));

    // Publishers
    lidar_pub_ = this->create_publisher<sensor_msgs::msg::LaserScan>("/lidar/scan", 10);
    odom_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/odom", 10);
...
private:
  ...
  rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr odom_raw_sub_;
  ...
  void config_callback(const std_msgs::msg::String::SharedPtr msg) {
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "Error parsing configuration string: %s", e.what());
    }
  }
  ...
  void lidar_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    if (packet_loss_enabled_ && should_drop_packet(packet_loss_rate_)) {
      RCLCPP_DEBUG(this->get_logger(), "LIDAR Packet Dropped (Loss rate: %.1f%%)", packet_loss_rate_ * 100);
    ...
    if (sensor_noise_enabled_) {
      for (size_t i = 0; i < out_msg->ranges.size(); ++i) {
        ...
        if (out_msg->ranges[i] >= out_msg->range_min && out_msg->ranges[i] <= out_msg->range_max) {
          double noise = generate_gaussian(0.0, sensor_noise_stddev_);
          out_msg->ranges[i] += static_cast<float>(noise);
          
          // Clamp values
          if (out_msg->ranges[i] < out_msg->range_min) out_msg->ranges[i] = out_msg->range_min;
          if (out_msg->ranges[i] > out_msg->range_max) out_msg->ranges[i] = out_msg->range_max;
        }
      }
    }
  ...
  /**
   * Pose/Odometry Callback: Intercepts position estimates, injecting systematic drift.
   */
  void odom_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    if (sensor_noise_enabled_) {
      out_msg->pose.position.x += generate_gaussian(0.0, sensor_noise_stddev_ * 0.5);
      out_msg->pose.position.y += generate_gaussian(0.0, sensor_noise_stddev_ * 0.5);
      out_msg->pose.position.z += generate_gaussian(0.0, sensor_noise_stddev_ * 0.5);
    }
